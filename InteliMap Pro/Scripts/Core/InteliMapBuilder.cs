...
namespace InteliMapPro
{
    ...
    {
		[Header("一般")]
		[Tooltip("ビルド用マップを解析する回数。大きいほどビルド時間は長く、生成はより正確になります。")]
        public int epochs = 1000;
		[Tooltip("学習するジェネレーターデータ。新規作成する場合は空にします。")]
        public GeneratorData generator;
		[Tooltip("タイルの『近傍』サイズ。近傍はタイルの決定に影響する周囲タイル群。例: 半径 1 は 3x3、半径 2 は 5x5。")]
        [Range(1, 10)] public int neighborhoodRadius = 2;

		[Header("マップ")]
		[Tooltip("解析しジェネレーター構築に用いるタイルマップの一覧。")]
		...
		[Header("詳細設定")]
		[Tooltip("ジェネレーターに関する詳細設定の集合。")]
        public GeneratorAdvanced generatorSettings;
		[Tooltip("学習処理に関する詳細設定の集合。")]
